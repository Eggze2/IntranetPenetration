...
void CRemoteClientDlg::OnBnClickedBtnFileinfo() {
	int ret = SendCommandPacket(1); // 处理发来的命令
	if (ret == -1) {
...
void CRemoteClientDlg::threadDownFile()
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		do {
			...
			if (nLength == 0) {
				AfxMessageBox(_T("文件长度为零，或者无法读取文件！"));
				break;
			}
			long long nCount = 0;
	...
	EndWaitCursor();	//结束光标等待状态
	MessageBox(_T("下载完成！"), _T("完成"));	//追加结束提示
}
